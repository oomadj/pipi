...
public class SlideHorizontalView extends RecyclerView {
    ...
    public SlideHorizontalView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public SlideHorizontalView(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    /**
     * 设置初始化时选中的位置,该方法必须在{ AutoLocateHorizontalView#setAdapter(android.support.v7.widget.RecyclerView.Adapter) }之前调用
     *
     * @param initPos 初始位置，如果位置超过了item的数量则默认选中最后一项item
     */
    public void setInitPos(int initPos) {
        //if (adapter != null) {   //xzx update
    ...
    /**
     * 设置每次显示多少个item,该方法必须在{ AutoLocateHorizontalView#setAdapter(v7.widget.RecyclerView.Adapter) }之前调用
     *
